package pe.edu.usmp.lavanderia.app.utils;

public class Constantes {
	
	public static String NAME_SYSTEM="SOFT-CLEAN";
	public static Integer NULL_COD = 0;
	public static Integer SUCCESS_COD = 1;
	public static String ICON_SUCCESS = "success";
	public static String ICON_ERROR = "error";
	public static String ICON_WARNING = "warning";
	public static String ICON_INFO = "info";
	public static String CONTRASENA_DEFAULT = "SofteClean1234@";

}
