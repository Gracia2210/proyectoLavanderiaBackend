...
public class SubservicioRepositoryImpl extends JdbcDaoSupport implements SubservicioRepository {
    ...
    public List<SubservicioResponse> listarSubserviciosPorServicioId(int servicioId) {
        String sql = "SELECT id, descripcion, monto, servicio_id,unidad,solo_seleccion,enabled FROM subservicio WHERE servicio_id = ?";
    ...
    public int insertarSubservicio(SubservicioResponse subservicio) {
        String sql = "INSERT INTO subservicio (descripcion, monto, servicio_id,unidad,solo_seleccion, enabled) VALUES (?, ?, ?, TRUE)";
        return jdbcTemplate.update(sql, subservicio.getDescripcion(), subservicio.getMonto(), subservicio.getServicioId(),subservicio.getUnidad(),subservicio.getSoloSeleccion());
    }

    @Override
    public int editarSubservicio(SubservicioResponse subservicio) {
        String sql = "UPDATE subservicio SET descripcion = ?, monto = ?, servicio_id = ?, unidad = ?,solo_seleccion = ? WHERE id = ?";
        return jdbcTemplate.update(sql, subservicio.getDescripcion(), subservicio.getMonto(), subservicio.getServicioId(),subservicio.getUnidad(),subservicio.getSoloSeleccion(), subservicio.getId());
    }

    @Override
    public SubservicioResponse buscarPorId(int id) {
        String sql = "SELECT id, descripcion, monto, servicio_id ,unidad,solo_seleccion,enabled FROM subservicio WHERE id = ?";
